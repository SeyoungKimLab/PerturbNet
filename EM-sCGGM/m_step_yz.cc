...
double Objective_yz(
	...
	return -logdet(Lambda) + traceProduct(Lambda, Szz) +
		2*traceProduct(Theta, Syz) + traceProduct(Theta, Gamma) +
...
// Uses Sigma
// Updates Theta, SyyTheta, and Gamma
void ThetaCoordinateDescent_yz(
	...
	if (options.sparse_Theta_cd) {
		...
		ThetaDense = MatrixXd(Theta);
		V.noalias() = ThetaDense * Sigma;
	}
	vector<long> columns;
	columns.reserve(r);
	for (long j = 0; j < r; ++j) {
	...
	if (!options.quiet) {
		...
		double time_theta_prep = toddiff(&start_time, &end_time);
		printf("      time ThetaCD prep: %.4f \n", time_theta_prep);
		fflush(stdout);
	}
	...
	if (!options.quiet) {
		printf("      time ThetaCD prep+cd: %.4f \n", time_theta_cd);
		fflush(stdout);
	}

	gettimeofday(&start_time, NULL);
	if (options.sparse_Theta_cd) {
		SyyTheta.noalias() = Syy * Theta;
		Gamma.noalias() = SyyTheta * Sigma;
	} else {
        printf("      about to ThetaDense\n");
        fflush(stdout);
		ThetaDense = MatrixXd(Theta);
        printf("      about to SyyTheta\n");
        fflush(stdout);
		SyyTheta.noalias() = Syy * ThetaDense;
        printf("      about to Gamma \n");
        fflush(stdout);
		Gamma.noalias() = SyyTheta * Sigma;
	}
	gettimeofday(&end_time, NULL);
...
void LambdaCoordinateDescent_yz(
	...
	if (!options.quiet) {
		printf("   starting YZ Lambda backtracking \n");
		fflush(stdout);
	}
	gettimeofday(&start_time, NULL);
...
void m_step_yz(
	...
	if (cholesky.info() != Success) { 
		if (!options.quiet) printf("   YZ Lambda0 not positive definite\n");
		return;
	}
	MatrixXd Sigma;
	...
	for (int t_outer = 0; t_outer < options.max_outer_iters; ++t_outer) {
		...
		gettimeofday(&mini_start_time, NULL);
		Psi.noalias() = Sigma * (Theta.transpose() * Gamma);
		...
		// CD for Theta, also updates SyyTheta and Gamma
		if (!options.quiet) {
			Lambda_sym = Lambda.selfadjointView<Upper>();
		...
		ThetaCoordinateDescent_yz(
			Szz, Syz, Syy, Theta, Sigma, SyyTheta, Gamma, lambda_y, 
