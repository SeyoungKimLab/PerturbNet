...
double Objective(
	...
	long n = X[0].size();
	double logdet = 0;
	int success = Lambda.ComputeLogdet(logdet, tol);	
...
double LambdaActiveSet(
	...
	for (long i = 0; i < q; i++) {
		...
		//printf("(%ld,%ld)=%f idx=%ld \n", i, i, Lambda.values[idx], idx);
		#pragma omp critical
		triplets.push_back(triplet);
	}
...
void LambdaCoordinateDescent(
	...
	for (vector<long>::iterator block_z_it = blocks_z_permuted.begin();
		...
		for (vector<long>::iterator block_q_it = blocks_q_permuted.begin();
			...
			//if (options.verbose >= 2 && block_z != block_q && curr_num_boundary) {
	...
	// If lsiters was 0 recently, only check PD-ness
	if (lState.numNoBacktracking > 10) {
	...
	double trDeltaSyy = Delta.traceProduct(Syy_in);
...
void m_step_xy(
	...
	for (int tOuter = 0; tOuter < options.max_outer_iters; tOuter++) {
		...
		if (options.verbose >= 1) {
			printf("   XY iteration %d: finished ThetaActive %ld \n",
				tOuter, Theta.nnz);
			fflush(stdout);
		}
		...
		if (options.verbose >= 1) {
			printf("   XY iteration %d: finished LambdaActive %ld \n",
				tOuter, Lambda.nnz);
			fflush(stdout);
		}
