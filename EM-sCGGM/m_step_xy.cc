...
struct Cache {
	Cache(const unsigned long cache_size_) {
		try {
			misses = 0;
			accesses = 0;
			hash.resize(cache_size_);
			data.resize(cache_size_);
			valid.resize(cache_size_, false);
		} catch (std::bad_alloc& ba) {
			fprintf(stderr, "cache could not fit in memory\n");
			exit(1);
		}
	}
	vector<long> hash;
...
inline void hash_combine(unsigned long & seed, unsigned long hashed_v) {
	// from boost::hash_combine
	seed ^= hashed_v + 0x9e3779b9 + (seed << 6) + (seed >> 2);
}

inline unsigned long hashpairs_std(unsigned long i_, unsigned long j_) {
	unsigned long seed = 0;
	unsigned long i = min(i_, j_);
	unsigned long j = max(i_, j_);
	hash_combine(seed, hash64shift(i));
	hash_combine(seed, hash64shift(j));
	return seed;
}
...
double Objective(
		const vector< vector<double> > &Syy,
	...
	//	traceProduct(R, Q));	
	return -1*logdet + Lambda.traceProduct(Syy) +
		2*(1.0/n)*Theta.traceProduct(X, eY) + traceProduct(R, Q) +
...
double ThetaActiveSet(
	...
	for (long i = 0; i < p; i++) {
		...
		for (long j = 0; j < q; j++) {
			double XtY_ij = 0;
			for (long nn = 0; nn < n; nn++) {
				XtY_ij += X[i][nn] * eY[j][nn];
			}
			double XtR_ij = 0;
			for (long nn = 0; nn < n; nn++) {
				XtR_ij += X[i][nn] * R[j][nn];
			}
			double G_ij = (2.0 / n) * XtY_ij + (2.0 / sqrt(n))*XtR_ij;
			double Theta_ij = 0;
...
void LambdaCoordinateDescent(
	...
	double trDeltaSyy = (1.0 / n) * Delta.traceProduct(Syy_in);
	...
	for (int lsiter = 0; lsiter < options.max_ls_iters; lsiter++) {
		...
		*/
		...
		for (long i = 0; i < n; i++) {
			...
			vector<double> R_i(q, 0);
			Lambda_alpha.ComputeAinvb(Q_i, R_i, options.obj_tol);
			for (long j = 0; j < q; j++) {
				R_alpha[j][i] = R_i[j];
			}
		}
		
		double trueLHS = Objective(Syy_in, Y, X, lambda_y, lambda_x, 
			Lambda_alpha, Theta, Q, R_alpha, options.obj_tol) - 
			Objective(Syy_in, Y, X, lambda_y, lambda_x,
			Lambda, Theta, Q, R, options.obj_tol);


		if (trueLHS <= alpha*RHS) {
			/*
			if (!options.quiet) {
			...
			*/
			...
			lState = LambdaState(
				logdetLambda_alpha, 0.0, numNoBacktracking);
			success = true;
			break;
		} 
		if (!options.quiet) {
			printf("      lsearch %d, alpha=%f, insufficient decrease=%f\n",
				lsiter, alpha, trueLHS);
		}
		alpha *= options.beta;
	}
...
void m_step_xy(
	...
	vector< vector<double> > Q(q, vector<double>(n, 0));
	Theta.computeLeftProduct(X, Q);
	...
	vector< vector<double> > R(q, vector<double>(n, 0));
	#pragma omp parallel for schedule(static)
	for (long i = 0; i < n; i++) {
		vector<double> Q_i(q, 0);
		for (long j = 0; j < q; j++) {
	...
	for (int tOuter = 0; tOuter < options.max_outer_iters; tOuter++) {
		...
		double subgradLambda = LambdaActiveSet(
			Syy, lambda_y, Lambda, R, options);
		gettimeofday(&mini_end_time, NULL);
		double lambda_active_time = toddiff(&mini_start_time, &mini_end_time);
		if (!options.quiet) {
			printf("   Iteration %d: finished LambdaActive_xy %ld \n",
				tOuter, Lambda.nnz);
			fflush(stdout);
		}
		...
		if (subgrad < options.tol*l1norm) {
			if (!options.quiet) {
				printf("   XY converged, subgrad=%f, norm=%f\n", subgrad, l1norm);
			}
			break;
		}
		...
		// Lambda CD
		if (!options.quiet) {
			printf("   XY iteration %d: starting LambdaCD \n", tOuter);
			fflush(stdout);
		}
		...
		// Theta CD
		if (!options.quiet) {
			printf("   XY iteration %d: starting ThetaCD \n", tOuter);
			fflush(stdout);
		}
		...
		ThetaCoordinateDescent(
			eY, X, lambda_y, lambda_x, Lambda, Theta, Q, R,
		...
		if (!options.quiet) {
			printf("   XY iteration %d, Lambda(subgrad=%f,active=%ld,norm=%f)\n",
